...
#Plot all circle through 3 points
for i in range(len(Points)):
   for j in range(i + 1, len(Points)):
      for k in range(j + 1, len(Points)):
         center=find_center_3_Points(Points[i],Points[j],Points[k])
         if((10 ** 18!=center[0]) or (10 ** 18!=center[1])):
            rad= computedistance(center, Points[i])
            draw3circle=plt.Circle(center,radius=rad,fill=False,edgecolor="yellow")
            plt.gca().add_artist(draw3circle)
